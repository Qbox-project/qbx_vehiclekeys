...
RegisterNetEvent('qb-vehiclekeys:client:GiveKeys', function(id)
    ...
    if targetVehicle then
        ...
        if HasKeys(targetPlate) then
            ...
            QBCore.Functions.NotifyV2({ description = Lang:t("notify.no_keys"), type = 'error' })
        end
    end
end)
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        ...
        QBCore.Functions.NotifyV2({ description = Lang:t("notify.not_near"), type = 'error' })
    end
end
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    ...
                    QBCore.Functions.NotifyV2({ description = Lang:t("notify.vehicle_locked"), type = 'inform' })
                else
                    TriggerServerEvent('qb-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
                    QBCore.Functions.NotifyV2({ description = Lang:t("notify.vehicle_unlocked"), type = 'inform' })
                end
...
function LockpickFinishCallback(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == cache.ped then
            ...
            QBCore.Functions.NotifyV2({ description = Lang:t("notify.vehicle_lockedpick"), type = 'success' })
...
function Hotwire(vehicle, plate)
    ...
    if lib.progressCircle({
        ...
        if (math.random() <= Config.HotwireChance) then
            ...
            QBCore.Functions.NotifyV2({ description = Lang:t("notify.failed_lockedpick"), type = 'error' })
        end
    ...
    if lib.progressCircle({
        ...
        if hasWeapon and isCarjacking then
            ...
            if math.random() <= carjackChance then
                ...
                QBCore.Functions.NotifyV2({ description = Lang:t("notify.carjack_failed"), type = 'error' })
                MakePedFlee(target)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            end
            isCarjacking = false
